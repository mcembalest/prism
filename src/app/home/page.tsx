...
import { createManualObservation } from '@/lib/screen-tracker'
...
export default function HomePage() {
  ...
  const totalNodesInGraph = flattenedActiveGraph.length
  const completedNodes = flattenedActiveGraph.filter((node) => node.status === 'completed')
  const upcomingNodes = flattenedActiveGraph
    .filter((node) => node.id !== activeCheckpoint.id)
    .filter((node) => node.status !== 'completed')
    .slice(0, 3)

  const recentNotes = sessionLog
    .filter((entry) => entry.checkpointId === (activeCheckpointId ?? activeTrack.id))
    .slice(-3)
    .reverse()
  ...
  const [selectedProduct, setSelectedProduct] = useState<string | null>(activeTrack?.product ?? null)
  ...
  useEffect(() => {
    ...
    if (activeTrack.product === selectedProduct) {
      return
    }
  ...
  }, [selectedProduct, activeTrack.product, productGroups, setActiveTrack])
  ...
  const emitProgressObservation = () => {
    createManualObservation(
      `Progress ping for ${activeCheckpoint.title}`,
      activeCheckpoint.id,
      'progress',
    )
  }

  const emitCompletionObservation = () => {
    createManualObservation(
      `Completion milestone recorded for ${activeCheckpoint.title}`,
      activeCheckpoint.id,
      'milestone',
    )
  }
      ...
      <Card>
        <CardHeader className="space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <CardTitle className="text-lg font-semibold">{activeTrack.title}</CardTitle>
            <Badge variant={getStatusBadgeVariant(activeTrack.status)} className="text-xs">
              {formatStatus(activeTrack.status)}
            </Badge>
          </div>
          <p className="text-sm text-muted-foreground max-w-2xl">
            {activeTrack.evaluationCriteria?.exampleTasks?.[0] ?? 'Stay focused on the current track and use the tutor as needed.'}
          </p>
        </CardHeader>
          ...
          {recentNotes.length > 0 ? (
            recentNotes.map((entry) => (
              <div key={entry.id} className="rounded-lg border border-border/60 p-3">
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>{entry.role === 'assistant' ? 'Assistant' : entry.role === 'user' ? 'You' : 'System'}</span>
                  <span>
                ...
                <p className="mt-1 text-sm leading-snug">{entry.content}</p>
              </div>
            ))
          ) : (
            <p className="text-sm text-muted-foreground">No log entries yet—interact with the tutor to record guidance.</p>
          )}

          <details className="rounded-lg border border-dashed border-muted-foreground/30 p-3 text-xs text-muted-foreground">
            <summary className="cursor-pointer font-medium text-foreground">Developer tools</summary>
            <div className="mt-2 flex flex-col gap-2 sm:flex-row">
              <Button variant="outline" size="sm" onClick={emitProgressObservation}>
                Emit progress event
              </Button>
              <Button size="sm" onClick={emitCompletionObservation}>
